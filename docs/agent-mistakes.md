...
| STATE-001 | Deferred/parallel execution lost required shared or lexical context | 5 | PR #5, 2026-08-24 | Classify cloned runtime state as local vs shared. Preserve logical ancestry and explicit assignment events across clones/caches, model waits and joins, and test nested deferred execution for deadlocks and caller-independent results. |
| SAFE-001 | Data crossed a trust or privacy boundary without validation | 8 | PR #5, 2026-08-24 | Treat workflow-derived paths and confidential inputs as untrusted. Preserve secret provenance across every binding path, prevent secrets from entering argv, redact structured leaves only when sufficiently distinctive, and test collisions, cleanup, over-redaction, and exposure boundaries. |
