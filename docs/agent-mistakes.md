...
| SAFE-001 | Data crossed a trust or privacy boundary without validation | 9 | PR #7, 2026-09-05 | Treat workflow-derived paths and confidential inputs as untrusted. Preserve secret provenance across every binding path, prevent secrets from entering argv, redact structured leaves only when sufficiently distinctive, and test collisions, cleanup, over-redaction, and exposure boundaries. |
...
| PARSE-002 | Malformed near-miss syntax was accepted as a different construct | 2 | PR #7, 2026-09-05 | For every new delimiter, test missing, doubled, truncated, quoted, and adjacent forms; reject unsupported forms at parse time. |
